...
context('Click', () => {
  ...
  it('allows clicking links in popovers', () => {
    ...
    cy.get('.tinyfoot__popover').should('not.have.class', 'is-scrollable')
    cy.get('.tinyfoot__content').should('not.have.attr', 'tabindex')
  ...
  it('popover layout upon resizing window before activation (#1702)', () => {
    ...
    cy.findByTitle('See Footnote 1').click()
    cy.get('.tinyfoot__content').invoke('width').should('be.greaterThan', 100)
  })
})
